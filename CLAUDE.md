...
- **`meeting-transcripts` substring-matches this server's ERROR TEXT — changing how Graph errors are rendered breaks its alarm** (mcp-hjw.1, 2026-08-17). `capture.py:158-181` `classify_error()` matches on `"403"` / `"forbidden"` / `"accessdenied"` / `"404"` / `"notfound"` / `"itemnotfound"` in `str(exc)`, and `error_shapes.py:47-66` deliberately PRESERVES the status code and tool name while scrubbing everything else. A committed baseline (`transcripts/_error_shapes.json`) holds literal shapes in the current `<status> — <code>: <innerError.code>: <message>` format produced by `_raise_with_graph_detail`, and `tests/test_error_shapes.py:73-90` replays a real 30-day cron-log corpus asserting it collapses to ≤8 shapes. **Consequence:** any change to `_raise_with_graph_detail`'s output format makes every shape read as novel, fires the capture alarm, and requires reseeding the baseline plus re-deriving `classify_error`. It is not a local refactor. This was also the strongest single argument against replacing this server wholesale (mcp-yip).
...
*Last 3 sessions. Deeper history: git log, beads, journal.
Cross-project orientation: `knowledge/self/work-context.md`.*

- **2026-08-17, session 2 —** **fastmcp 2.14.2 → 3.4.7 shipped, deployed and live** — plus the fork-vs-replace question settled. Epic **mcp-t66 CLOSED 7/7**: mcp-yip decided (keep the fork; Softeria added *alongside*, scoped to Teams chat — see MCP Gateway), mcp-m4e done, mcp-cgn done, and three latent gaps nobody had filed closed on the way (unbounded fastmcp floor, a bare `mcp.run()` that becomes env-overridable under 3.x, CI that could not detect lockfile drift, a test venv on 3.13 while the image ships 3.12). 10 commits `2c65c82..e34f019`, all CI green, digest `d11db20e`→`a90fbc6d`, verified by reading inside the image against the old one as a control on four dimensions. **`lupa` cleared by removal** — the bead wrote it off as unfixable since no patch exists, but 3.x drops the `fakeredis←pydocket` chain entirely. Rollback tag `microsoft-mcp:rollback-pre-e34f019`. Open: **mcp-mrj** (`search_emails` never got `_resolve_folder_id` — custom folder names fail there but work in `list_emails`), **mcp-c7a** (`_do_chunked_upload` is a 4th error-surfacing site mcp-xby's scope check missed), **mcp-7gu** (`list_emails` ordering), **mcp-s7p** (app-only — *not* solved by Softeria, which is delegated-only too). Theme: the measured estimate (~half a day) held exactly because it came from a real probe against 3.4.7 rather than from reading the changelog — and the same discipline caught that `dns-result-order` fixes were no-ops because `dns.lookup` returned *zero* IPv4 addresses, not badly-ordered ones. *Journal: 2026-08-17.*
- **2026-08-17, session 1 —** **mcp-xby shipped, merged and DEPLOYED.** New `_raise_with_graph_detail` centralises the 4xx Graph body parse and adds `innerError.code`, so the two transcript 403s are finally tellable apart — both carry `error.code: AccessDenied` and only innerError separates `GraphAccessToTranscriptsDisabled` (tenant Graph access off) from `SpeakerAttributionNotAllowed` (attribution off; same request succeeds unattributed). The bead's own scope check found **three** surfaces, not the one it named: `request` (had code+message, dropped innerError), `request_text` (the reported bug), `download_raw` (unreported, same class). `79fad5b` on main. Still load-bearing because **`meeting-transcripts` fingerprints failures by substring-matching this exact error format** (`error_shapes.py` + a committed `_error_shapes.json` baseline) — so any change to how Graph errors are rendered breaks the capture alarm, which is the strongest single argument found against ever swapping this server out. Theme: reverting each of the 3 call sites *separately* is what proved they were independently wired — mutating the shared helper once reddens everything and proves nothing about wiring. *Journal: 2026-08-17.*
